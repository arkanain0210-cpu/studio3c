...
export default function Home() {
  ...
  // 데이터 4배 복제 (선언은 딱 한 번만!)
  const dupLandscape = [...landscapeVideos, ...landscapeVideos, ...landscapeVideos, ...landscapeVideos];
  const dupVertical = [...verticalVideos, ...verticalVideos, ...verticalVideos, ...verticalVideos];

  const categories = ['ALL', '화보/프로필', '포트레이트', '스냅', '제품', '풍경'];
  const filteredPhotos = activeTab === 'ALL' ? photos : photos.filter(p => p.category === activeTab);

// 드래그가 끝났을 때 다시 애니메이션을 시작하는 함수
  ...
  return (
    <main className="min-h-screen bg-white text-[#1a1a1a] font-sans overflow-x-hidden">
      ...
      <section className="relative h-[85vh] w-full overflow-hidden bg-black text-white">
        ...
        <nav className="absolute top-0 left-0 right-0 p-10 flex justify-between items-center z-20 text-white text-left">
      ...
      {/* --- 2. STUDIO 3C: 무한 드래그 & 자동 재생 유지 섹션 --- */}
      <section id="studio3c" className="py-32 bg-black text-white overflow-hidden border-b border-zinc-900">
        ...
        <div className="relative flex overflow-hidden mb-12 h-[250px] md:h-[400px] cursor-grab active:cursor-grabbing">
          <motion.div 
            ...
            animate={controlsL} 
            initial={{ x: "0%" }}
            onViewportEnter={() => resumeAnimation(controlsL, 'left')}
            onDragStart={() => controlsL.stop()}
            onDragEnd={() => resumeAnimation(controlsL, 'left')}
            style={{ width: "max-content" }}
          >
            {dupLandscape.map((video, idx) => (
              <div key={`l-${idx}`} className="px-4 w-[400px] md:w-[600px] flex-shrink-0">
                <div onClick={() => setSelectedVideo(video.id)} className="relative aspect-video bg-zinc-900 overflow-hidden shadow-2xl group border border-white/5 cursor-pointer">
        ...
        <div className="relative flex overflow-hidden h-[320px] md:h-[500px] cursor-grab active:cursor-grabbing">
          <motion.div 
            ...
            animate={controlsV}
            initial={{ x: "-50%" }}
            onViewportEnter={() => resumeAnimation(controlsV, 'right')}
            onDragStart={() => controlsV.stop()}
            onDragEnd={() => resumeAnimation(controlsV, 'right')}
            style={{ width: "max-content" }}
          >
            {dupVertical.map((video, idx) => (
              <div key={`v-${idx}`} className="px-3 w-[180px] md:w-[280px] flex-shrink-0">
                <div onClick={() => setSelectedVideo(video.id)} className="relative aspect-[9/16] bg-zinc-900 overflow-hidden shadow-2xl group border border-white/5 cursor-pointer">
                  <Image src={`https://img.youtube.com/vi/${video.id}/maxresdefault.jpg`} alt={video.title} fill className="object-cover opacity-60 group-hover:opacity-100 transition-opacity duration-500" />
                </div>
              </div>
            ))}
      ...
      <section id="muguum" className="py-24 bg-white px-6">
        <div className="max-w-[1600px] mx-auto text-left">
          <div className="flex flex-col md:flex-row justify-between items-baseline mb-16">
            <div className="border-r-2 border-black pr-6 mb-8 md:mb-0">
              <h3 className="text-3xl font-serif italic tracking-tighter uppercase text-left">Studio Mug:um</h3>
              <p className="text-[10px] tracking-[0.4em] text-gray-400 mt-2 uppercase font-medium text-left">Photography Portfolio</p>
            </div>
        ...
        {selectedVideo && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setSelectedVideo(null)} className="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center p-4">
            <div className="relative w-full max-w-5xl aspect-video bg-black">
              <iframe src={`https://www.youtube.com/embed/${selectedVideo}?autoplay=1`} className="w-full h-full border-0" allow="autoplay; encrypted-media" allowFullScreen />
            </div>
          </motion.div>
        )}
        {selectedPhoto && (
      ...
      <footer className="py-20 text-center border-t border-gray-100 font-light italic">
        <p className="text-[9px] tracking-[0.5em] text-gray-400 uppercase">© 2026 Studio 3C & Studio Mug:um. Directed by Yu Ji-hwan.</p>
      </footer>
    </main>
  );
}